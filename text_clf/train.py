...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        train_batches = train_set.batch_iter()
        # loading devolopment data
        dev_set = DataLoad(data_path=FLAGS.train_data_path,
                             mode='train',
                             batch_size=FLAGS.batch_size,
                             num_epochs=FLAGS.num_epochs,
                             dev_sample_rate=FLAGS.dev_sample_rate)
        dev_batches = dev_set.batch_iter()
        ...
        for step in range(FLAGS.num_epochs):
            for _ in tqdm(range(1000)):
                ...
                sess.run(train_op, feed_dict={model.inputs: batch_x, model.labels: batch_y,
                                              model.dropout_keep_prob: FLAGS.dropout_keep_prob})
            dev_x, dev_y = next(dev_batches)
            loss, acc = sess.run([model.loss_op, model.accuracy_op],
                                 feed_dict={model.inputs: dev_x, model.labels: dev_y,
                                 model.dropout_keep_prob: FLAGS.dropout_keep_prob})
            print('setps:{} | loss:{} | acc:{}'.format(step, loss, acc))
